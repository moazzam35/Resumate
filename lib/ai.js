...
// Resolves the job title that should drive AI generation. The job title is the
// PRIMARY context for every resume section: prompts must never produce generic
// content when a title is known.
function resolveJobTitle(data) {
  return String(data?.targetJobTitle || data?.jobTitle || data?.title || "").trim();
}

// Fallback role label for prompts that need a noun ("for a ${X}").
function jobTitleForRole(data) {
  return resolveJobTitle(data) || "professional";
}

// Shared block appended to resume-content prompts so the model treats the job
// title (and optional job description) as the controlling context for every
// section it generates.
function jobContextBlock(data) {
  const jobTitle = resolveJobTitle(data);
  ...
  return `
TARGET JOB CONTEXT (this role controls ALL generated content):
${parts.join("\n")}

MANDATORY RULES:
- Every generated section must be directly relevant to the target job title above: its responsibilities, required skills, tools/technologies, terminology, and typical achievements.
- NEVER produce generic content that could apply to any profession. If the target is a Frontend Developer, focus on React, JavaScript, UI, responsive design, and front-end performance; if Backend Developer, focus on APIs, databases, server-side logic, and security; if Full Stack Developer, balance both; if Data Analyst, focus on SQL, Excel, Python, visualization, and reporting. Different job titles MUST produce meaningfully different content.
- Use ONLY skills, experience, and facts the candidate actually provided. Never invent job titles, employers, degrees, certifications, technologies, years of experience, projects, achievements, or metrics.
- When a job description is provided, it takes priority: Job Description > Job Title > the candidate's existing resume.
- Weave in relevant ATS keywords naturally. No keyword stuffing, no repetition.
`;
}

const AI_PROMPTS = {
  SUMMARY: (data) => `You are a professional resume writer. Generate a compelling professional summary for a ${jobTitleForRole(data)} with the following experience:
...
RULES:
- Make the summary specifically relevant to the exact job title: prioritize the skills, tools, and responsibilities that matter for that role and naturally include important ATS keywords for it.
- Do NOT simply repeat the job title back, and do NOT create a generic summary that could apply to any profession.
- Do NOT add technologies, experience, years, companies, achievements, or qualifications the user never provided.
- If the target job is a Frontend Developer, focus on React, JavaScript, UI, responsive design, and front-end performance; if Backend Developer, focus on APIs, databases, server-side logic, and security; if Full Stack Developer, balance both; if Data Analyst, focus on SQL, Excel, Python, visualization, and reporting. Different job titles must produce meaningfully different summaries.`,

  IMPROVE_EXPERIENCE: (data) => `Improve the following work experience description to be more impactful, professional, and directly relevant to the target job. Use strong action verbs, quantify achievements where possible, and follow best practices for resume writing.
${jobContextBlock(data)}
...
RULES:
- Every bullet must be relevant to the target job title and its required skills, technologies, and responsibilities. Do NOT write generic bullets that could apply to any role.
- Use this structure where appropriate: Action + What was done + Technology/Method + Result/Impact.
- Use ONLY facts implied by the provided description/highlights. Never invent employers, dates, metrics, or technologies.
- Keep the same job title and company EXACTLY as given.
...
Return the improved version as JSON with fields: description (string), highlights (array of strings). Make each highlight start with a strong action verb and include metrics where possible.`,
  ...
  REWRITE_BULLETS: (data) => `Rewrite the following bullet points to be more impactful, professional, and relevant to the target job. Each bullet should:
1. Start with a strong action verb
2. Include quantifiable results where possible
3. Be concise (1-2 lines max)
4. Focus on impact and results
5. Demonstrate skills and technologies relevant to the target job title
${jobContextBlock(data)}
...
RULES:
- Do NOT create generic bullets such as "Worked on various projects and collaborated with team members". Each bullet must show relevance to the target role.
- Use ONLY facts implied by the current bullets. Never invent metrics, technologies, or achievements.

Return the rewritten bullets as a JSON array of strings.`,

  GENERATE_EXPERIENCE: (data) => `You are a professional resume writer. Draft a complete WORK EXPERIENCE entry for the candidate applying to a ${jobTitleForRole(data)} role.

Target details provided by the candidate:
Company: ${data.company || "Not specified"}
Position: ${data.position || "Not specified"}
Location: ${data.location || "Not specified"}
Start date: ${data.startDate || "Not specified"}
End date: ${data.endDate || "Present"}
Notes / raw experience: ${data.description || "Not provided"}
${jobContextBlock(data)}

RULES:
- Write a description (2-3 sentences) and 3-5 highlight bullets for THIS role, directly relevant to the target job title and its required skills, tools, and responsibilities.
- Use the structure: Action + What was done + Technology/Method + Result/Impact.
- Use ONLY facts implied by the candidate's notes. Never invent employers, dates, technologies, achievements, or metrics.
- Keep the company name EXACTLY as given.

Return a JSON object with fields: description (string), highlights (array of strings).`, 

  GENERATE_SKILLS: (data) => `Based on the following role and experience, suggest relevant technical and soft skills that are directly useful for the target job.

Role: ${jobTitleForRole(data)}
...
RULES:
- Prioritize the core skills required for this exact role, then relevant supporting skills, then ATS-friendly keywords for the role.
- Only include skills that are provided by the user, clearly supported by their experience, or explicitly requested. Never add a skill simply because it is popular.
- Different job titles must produce different skill sets (Frontend → React, UI, responsive design; Backend → APIs, databases, security; Data Analyst → SQL, Excel, Python, visualization).
...
Suggest 15-20 relevant skills.`,
  ...
  GENERATE_PROJECTS: (data) => `Based on the following profile, suggest 2-3 project ideas that would be impressive on a resume and demonstrate the skills most relevant to the target job.
...
RULES:
- Interpret the projects according to the target job: for Frontend Developer emphasize UI, responsive design, reusable components, performance, and accessibility; for Backend Developer emphasize APIs, authentication, database design, and security; for Full Stack balance frontend and backend.
- Do NOT suggest projects that require skills the user has not provided.
...
Return a JSON array of project objects with fields: name (string), description (string, 2-3 sentences), technologies (array of strings), highlights (array of 2-3 bullet points).`,
  ...
  GENERATE_ACHIEVEMENTS: (data) => `Based on the following profile, suggest notable achievements and awards that would strengthen a resume for the target job.
...
RULES:
- Make every achievement relevant to the target role. Prefer measurable results when the user provided real numbers; never fabricate metrics.
- If no measurable information exists, describe the technical or business impact without inventing numbers.
...
Position: ${data.position || jobTitleForRole(data)}
Company: ${data.company}
Skills: ${data.skills || "Not specified"}
Experience: ${data.experience || "Not specified"}
...
2. Highlights relevant experience and skills tailored to the target job
3. Shows knowledge of the company
4. Ends with a clear call to action

RULES:
- Only reference skills and experience the applicant actually has. Do not invent credentials.
...
Return a JSON object with: correctedText (string), issues (array of objects with: original, corrected, explanation).`,
  ...
  ATS_KEYWORDS: (data) => `Analyze the following resume content and suggest ATS (Applicant Tracking System) keywords and phrases that should be included, tailored to the target job.
...
RULES:
- Keywords must be relevant to the exact target job title and its industry-standard terminology.
- Do NOT keyword-stuff; keywords should be naturally relevant.
...
RULES:
- Evaluate the resume against the target job title: flag missing skills, weak bullets, and generic content that fails to demonstrate the target role's requirements.
- Only suggest changes supported by the candidate's actual information.
...
- Naturally add the missing in-demand skills from the job description that align with the candidate's background, experience, and target job title. Do not add unrelated skills.
...
{"original":"<full original resume>","improved":"<full improved resume>","explanation":"<what was added and where>","scoreIncrease":<integer 0-15>}`,
  ...
  ATS_REWRITE_ACHIEVEMENTS: (data) => `You are a senior resume writer. Rewrite the achievements and bullet points below using strong action verbs and measurable results that matter for the target role.
...
Current role: ${jobTitleForRole(data)}
Skills: ${data.skills || "Not specified"}
Experience: ${data.experience || "Not specified"}
